...
const FINA3020Utils = {
    ...
    _deliver: function(payload) {
        ...
        // Spread a classroom-wide Live FX burst before entering the shared receiver lock.
        const start = payload.targetTab === 'Live_FX'
    ...
    _attemptDelivery: function(payload, attempt = 0) {
        const delays = payload.targetTab === 'Live_FX' ? [2000, 4000, 8000, 12000, 20000, 30000] : [2000, 6000];
        ...
        return this._post(payload).then(ack => {
            ...
            if (attempt < delays.length) {
                return new Promise(resolve => setTimeout(resolve, delays[attempt] + Math.floor(Math.random() * (payload.targetTab === 'Live_FX' ? 8000 : 1500))))
